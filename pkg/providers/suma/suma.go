...
import (
	...
	"github.com/cnrancher/autok3s/pkg/types/suma"
...
func (p *Suma) GetProviderName() string {
	return "suma"
}

func (p *Suma) GenerateClusterName() string {
	p.ContextName = p.Name
	return p.ContextName
}

func (p *Suma) GenerateManifest() []string {
	// no need to support
	return nil
}

func (p *Suma) GenerateMasterExtraArgs(cluster *types.Cluster, master types.Node) string {
	// no need to support.
	return ""
}

func (p *Suma) GenerateWorkerExtraArgs(cluster *types.Cluster, worker types.Node) string {
	// no need to support.
	return ""
}

func (p *Suma) CreateK3sCluster() (err error) {
...
func (p *Suma) JoinK3sNode() (err error) {
	if p.M == nil {
		p.M = new(syncmap.Map)
	}
	c := &types.Cluster{
...
func (p *Suma) rollbackInstance(ids []string) error {
	nodes := []types.Node{}
	for _, id := range ids {
		if node, ok := p.M.Load(id); ok {
			nodes = append(nodes, node.(types.Node))
...
func (p *Suma) DeleteK3sCluster(f bool) error {
	return p.CommandNotSupport("delete")
}

func (p *Suma) SSHK3sNode(ip string) error {
	return p.CommandNotSupport("ssh")
}

func (p *Suma) CommandNotSupport(commandName string) error {
	return fmt.Errorf("[%s] dose not support command: [%s]", p.GetProviderName(), commandName)
}

func (p *Suma) DescribeCluster(kubecfg string) *types.ClusterInfo {
	return &types.ClusterInfo{}
}

func (p *Suma) GetCluster(kubecfg string) *types.ClusterInfo {
	return &types.ClusterInfo{}
}

func (p *Suma) IsClusterExist() (bool, []string, error) {
	return false, []string{}, nil
}

func (p *Suma) SetConfig(config []byte) error {
...
func (p *Suma) SetOptions(opt []byte) error {
	sourceOption := reflect.ValueOf(&p.Options).Elem()
...
func (p *Suma) GetProviderOptions(opt []byte) (interface{}, error) {
...
func (p *Suma) syncNodesMap(ipList []string, master bool, ssh *types.SSH) {
